<template>
  <div class="objective-risk-page">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-3">
      <div class="lg:col-span-2 bg-white rounded-xl p-3 border border-gray-100">
        <div class="flex items-center justify-between mb-2">
          <h3 class="text-sm font-semibold text-gray-700">{{ t('suitability.objective.trendCardTitle') }}</h3>
          <div class="flex items-center gap-3 text-xs text-gray-400">
            <span class="flex items-center gap-1">
              <span class="w-3 h-0.5 bg-red-500 inline-block rounded"></span>{{ t('suitability.objective.qSeriesLegend') }}
            </span>
            <span class="flex items-center gap-1">
              <span class="w-3 h-0.5 bg-primary-500 inline-block rounded"></span>{{ t('suitability.objective.dSeriesLegend') }}
            </span>
          </div>
        </div>
    ...
    <div class="bg-white rounded-xl p-3 border border-gray-100 mb-3">
      <h3 class="text-sm font-semibold text-gray-700 mb-2">{{ t('suitability.objective.corrTitle') }}</h3>
      ...
      <div class="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-100 flex items-start gap-3">
        <div class="px-2 py-1 bg-primary-50 text-primary-600 rounded text-xs font-semibold whitespace-nowrap mt-0.5">
          {{ t('suitability.objective.corrInterpBadge') }}
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
      <div v-for="(metric, idx) in currentMetricCards" :key="idx"
        ...
        <div class="flex items-center gap-2 mb-2">
          ...
          <span class="text-xs text-gray-400">{{ tr(metric.label) }}</span>
        </div>
        <div class="text-lg font-bold text-gray-800 mb-0.5">{{ fmtMetric(metric.value) }}</div>
        ...
        <div class="text-xs text-gray-400 mt-0.5">{{ fmtMetric(metric.range) }}</div>
      </div>
    </div>
  </div>
</template>
...
const lastCorr = ref(null)
const corrMonths = ['2025-07','2025-08','2025-09','2025-10','2025-11','2025-12','2026-01','2026-02','2026-03','2026-04','2026-05','2026-06']
...
async function fetchAndApply(id) {
  ...
  try {
    ...
    const months = obj.months || corrMonths
    lastTrend.value = { months, trend }
    lastCorr.value = obj.corr || {}
    refreshTrend(months, trend)
    refreshCorr(lastCorr.value)
  } finally {
    loading.value = false
  }
}

function buildTrendOption(m, td) {
  const qLegend = t('suitability.objective.qSeriesLegend')
  const qPredLegend = t('suitability.objective.qPredSeriesLegend')
  const dLegend = t('suitability.objective.dSeriesLegend')
  const dPredLegend = t('suitability.objective.dPredSeriesLegend')
  return {
    tooltip: {
    ...
    series: [
      ...
      { name: dLegend, type: 'line', data: td.d, lineStyle: { color: '#165DFF', width: 2 }, itemStyle: { color: '#165DFF' }, symbol: 'circle', symbolSize: 6, smooth: true,
      ...
      { name: dPredLegend, type: 'line', data: td.dP, lineStyle: { color: '#165DFF', width: 2, type: 'dashed' }, itemStyle: { color: '#165DFF' }, symbol: 'diamond', symbolSize: 7, smooth: true, connectNulls: false },
    ]
  }
}

function buildCorrOption(c) {
  const riskLegend = t('suitability.objective.riskScoreLegend')
  const hsLegend = t('suitability.objective.hsIndexLegend')
  return {
    ...
    legend: { data: [riskLegend, hsLegend], top: 0, textStyle: { color: '#909399', fontSize: 11 } },
    grid: { top: 35, right: 60, bottom: 30, left: 50 },
    xAxis: { type: 'category', data: corrMonths, axisLine: { lineStyle: { color: '#e5eaf2' } }, axisTick: { show: false }, axisLabel: { color: '#909399', fontSize: 11 } },
    yAxis: [
      { type: 'value', min: 1, max: 5, interval: 1, name: t('suitability.objective.riskAxisName'), nameTextStyle: { color: '#909399', fontSize: 11 }, axisLine: { show: false }, axisTick: { show: false }, splitLine: { lineStyle: { color: '#f2f3f5', type: 'dashed' } }, axisLabel: { color: '#909399', fontSize: 11, formatter: 'C{value}' } },
      { type: 'value', min: 3400, max: 4500, name: t('suitability.objective.hsAxisName'), nameTextStyle: { color: '#909399', fontSize: 11 }, axisLine: { show: false }, axisTick: { show: false }, splitLine: { show: false }, axisLabel: { color: '#909399', fontSize: 11 } }
    ],
    series: [
      { name: riskLegend, type: 'line', data: c.risk, lineStyle: { color: '#165DFF', width: 2 }, itemStyle: { color: '#165DFF' }, symbol: 'circle', symbolSize: 6, smooth: true },
      { name: hsLegend, type: 'line', yAxisIndex: 1, data: c.hs, lineStyle: { color: '#E6A23C', width: 2 }, itemStyle: { color: '#E6A23C' }, symbol: 'circle', symbolSize: 6, smooth: true },
    ]
  }
}

function refreshTrend(m, t) { if (trendChart) { try { trendChart.setOption(buildTrendOption(m, t), { notMerge: true }); trendChart.resize() } catch (e) { console.error('refreshTrend failed', e) } } }
function refreshCorr(c) { if (correlationChart) { try { correlationChart.setOption(buildCorrOption(c), { notMerge: true }); correlationChart.resize() } catch (e) { console.error('refreshCorr failed', e) } } }
...
watch(locale, () => {
  if (trendChart && lastTrend.value) {
    nextTick(() => {
      refreshTrend(lastTrend.value.months, lastTrend.value.trend)
      trendChart?.resize()
    })
  }
  if (correlationChart && lastCorr.value) {
    correlationChart.setOption(buildCorrOption(lastCorr.value), { notMerge: true })
...
onMounted(() => {
  try {
    ...
    if (correlationChartRef.value) correlationChart = echarts.init(correlationChartRef.value)
  } catch (e) {
    console.error('echarts init failed', e)
  }
  window.addEventListener('resize', handleResize)
  fetchAndApply(currentCustomerId.value)
  nextTick(() => { trendChart?.resize(); correlationChart?.resize() })
})
